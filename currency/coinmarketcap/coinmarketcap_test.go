...
func TestGetCryptocurrencyHistoricalQuotesRejectsMissingResult(t *testing.T) {
	...
	assert.ErrorIs(t, err, common.ErrNoResponse, "historical quote response without the requested ID should return no response")
}
...
func TestGetExchangeLatestQuotesWithoutBinance(t *testing.T) {
	...
	require.NoError(t, err, "latest exchange quote request must not error")
	require.Len(t, result.Exchanges, 1, "latest exchange quotes must retain every result")
	assert.Zero(t, result.Binance, "Binance compatibility field should remain empty")
}
...
func TestGetExchangeHistoricalQuotesRejectsMissingResult(t *testing.T) {
	...
	assert.ErrorIs(t, err, common.ErrNoResponse, "historical exchange response without the requested ID should return no response")
}
...
func TestGetPriceConversionPlanAccess(t *testing.T) {
	...
	assert.ErrorIs(t, err, errFunctionUseNotAllowed, "an unset plan should not allow latest price conversion")
	...
	assert.NoError(t, err, "Builder-plan historical conversion should not error")
}
...
func TestSetAccountPlan(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "basic", accountPlan: "basic", expected: Basic},
		{name: "builder", accountPlan: "builder", expected: Builder},
		{name: "startup", accountPlan: "startup", expected: Startup},
		{name: "growth", accountPlan: "growth", expected: Growth},
		{name: "professional", accountPlan: "professional", expected: Professional},
		{name: "enterprise", accountPlan: "enterprise", expected: Enterprise},
		{name: "normalised", accountPlan: " Growth ", expected: Growth},
		{name: "legacy hobbyist", accountPlan: "hobbyist", err: errInvalidAccountPlan},
		{name: "legacy standard", accountPlan: "standard", err: errInvalidAccountPlan},
		{name: "unknown", accountPlan: "unknown", err: errInvalidAccountPlan},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			...
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err, "SetAccountPlan must reject invalid or obsolete account plans")
				assert.Equal(t, Enterprise, c.Plan, "an invalid plan should not change the current account plan")
				return
			}
			require.NoError(t, err, "SetAccountPlan must not error")
			assert.Equal(t, tc.expected, c.Plan, "account plan should match")
		})
	}
}
...
func TestNewFromSettingsAndSetupDisabled(t *testing.T) {
	...
	assert.True(t, client.Verbose, "Setup should apply the verbose setting")
	assert.Equal(t, "x", client.APIkey, "Setup should apply the API key")
...
func TestSetupRejectsInvalidPlanWithoutMutation(t *testing.T) {
	...
	assert.True(t, client.Enabled, "Setup should preserve the enabled state after an error")
	assert.True(t, client.Verbose, "Setup should preserve the verbose state after an error")
	assert.Equal(t, "old-api-key", client.APIkey, "Setup should preserve the API key after an error")
	assert.Equal(t, Enterprise, client.Plan, "Setup should preserve the account plan after an error")
}
...
func TestQuoteMapUnmarshal(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				assert.ErrorIs(t, err, common.ErrInvalidResponse, "invalid quote collection should return the response error")
				return
			}
			require.NoError(t, err, "quote collection must unmarshal")
			assert.Equal(t, tc.expected, actual, "quote collection should match")
		})
	}
}
...
func TestCoinmarketcapAccountPlanGatesSynthetic(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := tc.invoke(c)
			require.NoError(t, err, "documented minimum plan must allow the endpoint")

			c.Plan = tc.minimumPlan >> 1
			err = tc.invoke(c)
			assert.ErrorIs(t, err, errFunctionUseNotAllowed, "lower plan should return the plan error")
		})
	}
}
