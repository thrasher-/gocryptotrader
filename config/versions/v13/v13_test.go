...
func TestUpgradeConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.JSONEq(t, tc.expected, string(out), "UpgradeConfig should only migrate deprecated plan names")

			outAgain, err := new(v13.Version).UpgradeConfig(t.Context(), out)
			require.NoError(t, err, "UpgradeConfig must be idempotent")
			assert.Equal(t, out, outAgain, "a second upgrade should not change the config")
		})
	}
}
...
func TestDowngradeConfig(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.JSONEq(t, tc.expected, string(out), "DowngradeConfig should restore only renamed plans")

			outAgain, err := new(v13.Version).DowngradeConfig(t.Context(), out)
			require.NoError(t, err, "DowngradeConfig must be idempotent")
			assert.Equal(t, out, outAgain, "a second downgrade should not change the config")
		})
	}
}
...
func TestRegisteredMigration(t *testing.T) {
	...
	assert.JSONEq(t, `{"version":13,"currencyConfig":{"cryptocurrencyProvider":{"accountPlan":"builder","apiKey":"keep"}}}`, string(out), "Deploy should migrate Hobbyist to Builder and set version 13")
	...
	require.NoError(t, err, "Deploy must apply the registered v13 downgrade")
	assert.JSONEq(t, `{"version":12,"currencyConfig":{"cryptocurrencyProvider":{"accountPlan":"standard","apiKey":"keep"}}}`, string(out), "Deploy should restore Growth to Standard and set version 12")

	input = []byte(`{"version":12,"currencyConfig":{"cryptocurrencyProvider":{"accountPlan":"accountPlan","apiKey":"keep"}}}`)
	out, err = versions.Manager.Deploy(t.Context(), input, 13)
	require.NoError(t, err, "Deploy must migrate the legacy account plan placeholder")
	assert.JSONEq(t, `{"version":13,"currencyConfig":{"cryptocurrencyProvider":{"accountPlan":"basic","apiKey":"keep"}}}`, string(out), "Deploy should replace the legacy placeholder with Basic and set version 13")
}
