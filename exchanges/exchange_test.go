...
func TestUpdatePairs(t *testing.T) {
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true, false), "UpdatePairs should not error when updating available pairs")
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false, false), "UpdatePairs should not error when updating enabled pairs")

	// Test updating the same new products, diff should be 0
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true, false), "UpdatePairs should not error when reapplying available pairs")
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true, true), "UpdatePairs should not error when forced updating available pairs")
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false, false), "UpdatePairs should not error when updating enabled pairs for exchange")

	// Test updating the same new products, diff should be 0
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false, false), "UpdatePairs should not error when reapplying enabled pairs")

	// Test force updating to only one product
	exchangeProducts, err = currency.NewPairsFromStrings([]string{"btcusd"})
	require.NoError(t, err, "NewPairsFromStrings must not return error for forced enabled pair")
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false, true), "UpdatePairs should not error when forcing enabled pairs")
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false, false), "UpdatePairs should not error when excluding BTC")
