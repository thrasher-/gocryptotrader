...
func TestGetURL(t *testing.T) {
	...
	require.NoError(t, err, "SetDefaultEndpoints must not return error")
...
func TestSetClientProxyAddress(t *testing.T) {
	...
	require.NoError(t, err, "request.New must not return error")
...
func TestSetAutoPairDefaults(t *testing.T) {
	...
	require.NoError(t, err, "GetExchangeConfig must not return error")
...
func TestGetClientBankAccounts(t *testing.T) {
	...
	require.NoError(t, err, "LoadConfig must not return error")
...
func TestGetExchangeBankAccounts(t *testing.T) {
	...
	require.NoError(t, err, "GetExchangeBankAccounts must not return error")
	assert.Equal(t,
...
func TestSetCurrencyPairFormat(t *testing.T) {
	...
	require.NoError(t, err, "SetCurrencyPairFormat must not return error")
...
func TestLoadConfigPairs(t *testing.T) {
	...
	require.NoError(t, err, "GetEnabledPairs must not return error")

	p := pairs[0].Format(pFmt).String()
	assert.Equal(t, "BTC^USD", p, "pairs[0].Format should return BTC^USD")

	avail, err := b.GetAvailablePairs(asset.Spot)
	require.NoError(t, err, "GetAvailablePairs must not return error")

	format, err := b.FormatExchangeCurrency(avail[0], asset.Spot)
	require.NoError(t, err, "FormatExchangeCurrency must not return error")
	...
	require.NoError(t, b.SetConfigPairs(), "SetConfigPairs must not return error")
	// Test four things:
	// 1) XRP-USD is set
	// 2) pair format is set for RequestFormat
	// 3) pair format is set for ConfigFormat
	...
	require.NoError(t, err, "CurrencyPairs.Get must not return error")
...
func TestSetupDefaults(t *testing.T) {
	...
	require.NoError(t, b.SetupDefaults(&cfg), "SetupDefaults must not return error")
	assert.Equal(t, "15s", cfg.HTTPTimeout.String(), "SetupDefaults should set HTTP timeout to 15s")

	// Test custom HTTP timeout is set
	cfg.HTTPTimeout = time.Second * 30
	require.NoError(t, b.SetupDefaults(&cfg), "SetupDefaults must not return error")
	assert.Equal(t, "30s", cfg.HTTPTimeout.String(), "HTTPTimeout.String should return 30s")
	...
	require.NoError(t, err, "CurrencyPairs.Get must not return error")
	assert.True(t, ps.Enabled.Contains(btcusdPair, true), "CurrencyPairs.Enabled.Contains should contain default pair")

	// Test websocket support
	b.Websocket = websocket.NewManager()
	b.Features.Supports.Websocket = true
	require.NoError(t, b.Websocket.Setup(&websocket.ManagerSetup{
	...
	}), "Websocket.Setup must not return error")
	require.NoError(t, b.Websocket.Enable(), "Websocket.Enable must not return error")
	assert.True(t, b.IsWebsocketEnabled(), "IsWebsocketEnabled should return true")
}
...
func TestUpdatePairs(t *testing.T) {
	...
	require.NoError(t, err, "NewPairsFromStrings must not return error")
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true), "UpdatePairs should not error when updating enabled pairs")
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false), "UpdatePairs should not error when updating available pairs")

	// Test updating the same new products, diff should be 0
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true), "UpdatePairs should not error when reapplying enabled pairs")
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, true), "UpdatePairs should not error when forcing enabled pairs")
	...
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false), "UpdatePairs should not error when updating available pairs for exchange")

	// Test updating the same new products, diff should be 0
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false), "UpdatePairs should not error when reapplying available pairs")

	// Test force updating to only one product
	exchangeProducts, err = currency.NewPairsFromStrings([]string{"btcusd"})
	require.NoError(t, err, "NewPairsFromStrings must not return error for forced available pair")
	assert.NoError(t, UAC.UpdatePairs(exchangeProducts, asset.Spot, false), "UpdatePairs should not error when forcing available pairs")
	...
	assert.NoError(t, err, "UpdatePairs should not error for single available pair")

	err = UAC.UpdatePairs(currency.Pairs{btcusdPair}, asset.Spot, true)
	assert.NoError(t, err, "UpdatePairs should not error for single enabled pair")
	...
	require.NoError(t, err, "GetEnabledPairs must not error")
	assert.True(t, uacPairs.Contains(btcusdPair, true), "GetEnabledPairs should contain BTC-USD")
	...
	require.NoError(t, UAC.UpdatePairs(pairs, asset.Spot, true))
	...
	require.NoError(t, UAC.UpdatePairs(pairs, asset.Spot, false))
	...
	require.NoError(t, err, "NewPairFromString must not return error")
	...
	uacEnabledPairs, err = UAC.GetEnabledPairs(asset.Spot)
	require.NoError(t, err, "GetEnabledPairs must not return error after final update")
	...
	require.NoError(t, UAC.UpdatePairs(currency.Pairs{}, asset.Spot, true), "Purging enabled pairs must not error")

	pairs, err = UAC.GetEnabledPairs(asset.Spot)
	require.NoError(t, err, "GetEnabledPairs must not return error after purge")
	require.Empty(t, pairs, "Enabled pairs must be empty after purge")

	err = UAC.UpdatePairs(currency.Pairs{}, asset.Spot, false)
	require.ErrorIs(t, err, currency.ErrCurrencyPairsEmpty, "Purging available pairs must return ErrCurrencyPairsEmpty")

	avail, err := UAC.GetAvailablePairs(asset.Spot)
	require.NoError(t, err, "GetAvailablePairs must not return error after failed purge")
	assert.NotEmpty(t, avail, "Available pairs should remain after failed purge")
}
...
func TestIsWebsocketEnabled(t *testing.T) {
	...
	var b Base
	assert.False(t, b.IsWebsocketEnabled(), "IsWebsocketEnabled should return false when not configured")
	...
	assert.True(t, b.IsWebsocketEnabled(), "IsWebsocketEnabled should return true after setup")
}
...
func TestGetFormattedPairAndAssetType(t *testing.T) {
	...
	require.NoError(t, b.SetCurrencyPairFormat(), "SetCurrencyPairFormat must not return error")
	b.Config.CurrencyPairs.UseGlobalFormat = true
	b.CurrencyPairs.UseGlobalFormat = true
	pFmt := &currency.PairFormat{
		Delimiter: "#",
...
func TestSetAPIURL(t *testing.T) {
	...
	require.NoError(t, err, "Endpoints.GetURL must not return error")
...
func TestSetRequester(t *testing.T) {
	...
	require.NoError(t, b.SetRequester(requester), "SetRequester must not return error")
