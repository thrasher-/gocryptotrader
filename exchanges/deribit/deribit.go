...
func validateBlockRFQTradeAllocations(tradeAllocations []BlockRFQTradeAllocation) error {
	for i := range tradeAllocations {
		if tradeAllocations[i].UserID == 0 {
...
func (e *Exchange) AcceptBlockRFQ(ctx context.Context, blockRFQID uint64, amount float64, tradeAllocations []BlockRFQTradeAllocation) (*BlockRFQAcceptResponse, error) {
	if blockRFQID <= 0 {
		return nil, errMissingBlockRFQID
	}
	if amount < 0 {
	...
	if len(tradeAllocations) > 0 {
		err = setJSONParam(params, "trade_allocations", tradeAllocations)
		if err != nil {
			return nil, err
		}
	}
