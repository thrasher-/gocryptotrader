...
func (e *Exchange) WSCreateBlockRFQ(ctx context.Context, req *CreateBlockRFQRequest) (*BlockRFQData, error) {
	...
	checkedLegs, err := validateBlockRFQLegs(req.Legs, true)
	if err != nil {
		return nil, err
	}
...
func (e *Exchange) WSEditBlockRFQQuote(ctx context.Context, req *EditBlockRFQQuoteRequest) (*BlockRFQQuote, error) {
	...
	if req.BlockRFQQuoteID <= 0 {
		return nil, errMissingBlockRFQQuoteID
	}
	normalisedDirection, err := validateBlockRFQDirection(req.Direction)
	if err != nil {
		return nil, err
	...
	input := map[string]any{
		"block_rfq_quote_id": req.BlockRFQQuoteID,
	}
	if req.Amount > 0 {
		input["amount"] = req.Amount
	}
...
func (e *Exchange) WSCancelAllBlockRFQQuotes(ctx context.Context, blockRFQID uint64, detailed bool) (*BlockRFQQuoteCancelResponse, error) {
	...
	resp := &BlockRFQQuoteCancelResponse{}
...
func (e *Exchange) WSAcceptBlockRFQ(ctx context.Context, blockRFQID uint64, amount float64, tradeAllocations []BlockRFQTradeAllocation) (*BlockRFQAcceptResponse, error) {
	if blockRFQID <= 0 {
		return nil, errMissingBlockRFQID
	}
	if amount < 0 {
	...
	err := validateBlockRFQTradeAllocations(tradeAllocations)
	if err != nil {
		return nil, err
	}
	input := map[string]any{
		"block_rfq_id": blockRFQID,
	}
	if amount > 0 {
		input["amount"] = amount
	}
	if len(tradeAllocations) > 0 {
		input["trade_allocations"] = tradeAllocations
	}
	resp := &BlockRFQAcceptResponse{}
...
func (e *Exchange) WSRetrieveBlockRFQTrades(ctx context.Context, ccy currency.Code, continuation string, count uint64) (*BlockRFQTradeTapeResponse, error) {
	...
	resp := &BlockRFQTradeTapeResponse{}
