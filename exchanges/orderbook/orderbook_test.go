...
func TestCheckAlignment(t *testing.T) {
	...
	err := checkAlignment(itemWithFunding, true, true, false, false, descending, "Bitfinex")
	if err != nil {
		t.Error(err)
	}
	err = checkAlignment(itemWithFunding, false, true, false, false, descending, "Bitfinex")
	require.ErrorIs(t, err, ErrPriceZero)

	err = checkAlignment(itemWithFunding, true, true, false, false, descending, "Binance")
	require.ErrorIs(t, err, ErrPriceZero)

	itemWithFunding[0].Price = 1337
	err = checkAlignment(itemWithFunding, true, true, false, true, descending, "Binance")
