...
func validate(b *Book) error {
	...
	if b.ChecksumStringRequired && (len(b.BidDigits) != len(b.Bids) || len(b.AskDigits) != len(b.Asks)) {
		return fmt.Errorf("%s %s %s: %w", b.Exchange, b.Pair, b.Asset, errChecksumStringNotSet)
	}
	if b.ChecksumStringRequired {
		if err := checkDigits(b.BidDigits); err != nil {
			return fmt.Errorf(bidLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
		}
		if err := checkDigits(b.AskDigits); err != nil {
			return fmt.Errorf(askLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
		}
	}
	err := checkAlignment(b.Bids, b.IsFundingRate, b.PriceDuplication, b.IDAlignment, descending, b.Exchange)
	if err != nil {
		return fmt.Errorf(bidLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
	}
	err = checkAlignment(b.Asks, b.IsFundingRate, b.PriceDuplication, b.IDAlignment, ascending, b.Exchange)
	if err != nil {
		return fmt.Errorf(askLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
	}
	return nil
...
func validateAround(b *Book, bids, asks Levels) error {
	...
	err := checkAlignmentAround(b.Bids, bids, b.IsFundingRate, b.PriceDuplication, b.IDAlignment, descending, b.Exchange)
	if err != nil {
		return fmt.Errorf(bidLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
	}
	err = checkAlignmentAround(b.Asks, asks, b.IsFundingRate, b.PriceDuplication, b.IDAlignment, ascending, b.Exchange)
	if err != nil {
		return fmt.Errorf(askLoadBookFailure, b.Exchange, b.Pair, b.Asset, err)
	}
	return nil
...
func checkAlignmentAround(depth, touched Levels, fundingRate, priceDuplication, isIDAligned, isAscending bool, exch string) error {
	for x := range touched {
		...
		if err := checkAlignment(depth[lo:hi], fundingRate, priceDuplication, isIDAligned, isAscending, exch); err != nil {
			return err
		}
	}
	return nil
...
// checkDigits holds each digit to the emptiness test the strings on Level once got. A digit the
// venue did not write hashes to the wrong checksum, so a side checksummed from them cannot carry one.
func checkDigits(digits []LevelDigits) error {
	for i := range digits {
		if digits[i].Price == "" || digits[i].Amount == "" {
...
func checkAlignment(depth Levels, fundingRate, priceDuplication, isIDAligned, isAscending bool, exch string) error {
	for i := range depth {
		if depth[i].Price == 0 {
			switch {
