...
func randomSide(r *rand.Rand, length int, isAscending bool) Levels {
	...
	for i := range l {
		...
		l[i] = Level{Price: price, Amount: 0.01 + r.Float64(), ID: int64(i + 1), Period: 1}
	}
	return l
}
...
func randomUpdates(r *rand.Rand, side Levels, count int) Levels {
	...
	for i := range updts {
		u := Level{Amount: 0.01 + r.Float64(), ID: int64(r.IntN(64)), Period: 1}
		switch {
...
func TestValidateAroundMatchesValidate(t *testing.T) {
	...
	for range 50_000 {
		...
		bids := bidLevels{}
		asks := askLevels{}
		bids.load(randomSide(r, r.IntN(64), descending), nil)
		asks.load(randomSide(r, r.IntN(64), ascending), nil)
		...
		bids.updateInsertByPrice(bidUpdts, nil, maxDepth)
		asks.updateInsertByPrice(askUpdts, nil, maxDepth)
...
// TestSnapshotAndUpdatePreconditions covers the premises the windowed check rests on: a snapshot is
// validated as it is installed, an update carries a digit for every level it names, and a checksum
// is verified whenever a generator is supplied rather than only when the expected value is non-zero.
func TestSnapshotAndUpdatePreconditions(t *testing.T) {
	t.Parallel()
	...
	// a retrieved book must satisfy the validation its own options ask for
	d = NewDepth(id)
	d.options.validateOrderbook = true
	d.options.checksumStringRequired = true
	require.NoError(t, d.LoadSnapshot(&Book{
		Bids: Levels{{Price: 100, Amount: 1}}, Asks: Levels{{Price: 101, Amount: 1}},
		BidDigits:   []LevelDigits{{Price: "100", Amount: "1"}},
		AskDigits:   []LevelDigits{{Price: "101", Amount: "1"}},
		LastUpdated: time.Now(),
	}))
	got, err := d.Retrieve()
	require.NoError(t, err)
	got.ValidateOrderbook = true
	require.NoError(t, got.Validate(), "a retrieved book must carry the digits its options require")

	// an update naming levels without digits must be refused rather than blanking good ones
	require.ErrorIs(t, d.ProcessUpdate(&Update{
		UpdateTime: time.Now(), Bids: Levels{{Price: 100, Amount: 2}},
	}), errChecksumStringNotSet, "an update must carry a digit for every level")
...
func TestNonFiniteValuesRejected(t *testing.T) {
	...
	for name, level := range map[string]Level{
		...
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			assert.ErrorIs(t, checkAlignment(Levels{level}, false, true, false, ascending, "test"),
...
// TestChecksumDigitsMustNotBeEmpty covers the per level emptiness test that the strings on Level
// used to get. A length matched sidecar holding an empty digit hashes to the wrong checksum.
func TestChecksumDigitsMustNotBeEmpty(t *testing.T) {
	t.Parallel()

	newBook := func(bidDigits, askDigits []LevelDigits) *Book {
		return &Book{
			Exchange: "test", Pair: currency.NewBTCUSD(), Asset: asset.Spot, LastUpdated: time.Now(),
			ChecksumStringRequired: true,
			Bids:                   Levels{{Price: 100, Amount: 1}},
			Asks:                   Levels{{Price: 101, Amount: 1}},
			BidDigits:              bidDigits,
			AskDigits:              askDigits,
		}
	}

	good := []LevelDigits{{Price: "100", Amount: "1"}}
	require.NoError(t, validate(newBook(good, []LevelDigits{{Price: "101", Amount: "1"}})),
		"a book with digits on every level must validate")

	for name, digits := range map[string][]LevelDigits{
		"empty price":  {{Price: "", Amount: "1"}},
		"empty amount": {{Price: "100", Amount: ""}},
		"both empty":   {{Price: "", Amount: ""}},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
			err := validate(newBook(digits, []LevelDigits{{Price: "101", Amount: "1"}}))
			assert.ErrorIsf(t, err, errChecksumStringNotSet, "%s should be rejected", name)
		})
	}

	// The windowed check must reject it too, or an update could install one for a thousand updates
	d := NewDepth(id)
	d.AssignOptions(newBook(good, []LevelDigits{{Price: "101", Amount: "1"}}))
	require.NoError(t, d.LoadSnapshot(newBook(good, []LevelDigits{{Price: "101", Amount: "1"}})))
	d.validateOrderbook = true
	err := d.ProcessUpdate(&Update{
		UpdateTime: time.Now(),
		Bids:       Levels{{Price: 99, Amount: 1}},
		BidDigits:  []LevelDigits{{Price: "", Amount: "1"}},
	})
	assert.ErrorIs(t, err, errChecksumStringNotSet, "the windowed check should reject an empty digit too")
}
