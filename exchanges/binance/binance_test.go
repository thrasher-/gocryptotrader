...
func TestWrapperGetServerTime(t *testing.T) {
	...
	require.NoError(t, err, "GetServerTime must not error for spot")
	require.False(t, st.IsZero(), "Server time for spot must not be zero")

	st, err = e.GetServerTime(t.Context(), asset.USDTMarginedFutures)
	require.NoError(t, err, "GetServerTime must not error for USDT margined futures")
	require.False(t, st.IsZero(), "Server time for USDT margined futures must not be zero")

	st, err = e.GetServerTime(t.Context(), asset.CoinMarginedFutures)
	require.NoError(t, err, "GetServerTime must not error for coin margined futures")
	require.False(t, st.IsZero(), "Server time for coin margined futures must not be zero")
}

func TestUpdateTicker(t *testing.T) {
	t.Parallel()
	r, err := e.UpdateTicker(t.Context(), testPairMapping, asset.Spot)
	assert.NoError(t, err, "UpdateTicker should not error for spot")
	if assert.NotNil(t, r, "Ticker should not be nil") {
		assert.Equal(t, currency.DOGE, r.Pair.Base, "Pair base should be DOGE")
		assert.Equal(t, currency.USDT, r.Pair.Quote, "Pair quote should be USDT")
	}

	tradablePairs, err := e.FetchTradablePairs(t.Context(), asset.CoinMarginedFutures)
	assert.NoError(t, err, "FetchTradablePairs should not error for coin margined futures")
	...
	assert.NoError(t, err, "UpdateTicker should not error for coin margined futures")

	usdtMarginedPairs, err := e.FetchTradablePairs(t.Context(), asset.USDTMarginedFutures)
	assert.NoError(t, err, "FetchTradablePairs should not error for USDT margined futures")
	...
	assert.NoError(t, err, "UpdateTicker should not error for USDT margined futures")
}

func TestUpdateTickers(t *testing.T) {
	err := e.UpdateTickers(t.Context(), asset.Spot)
	assert.NoError(t, err, "UpdateTickers should not error for spot")

	err = e.UpdateTickers(t.Context(), asset.CoinMarginedFutures)
	assert.NoError(t, err, "UpdateTickers should not error for coin margined futures")

	err = e.UpdateTickers(t.Context(), asset.USDTMarginedFutures)
	assert.NoError(t, err, "UpdateTickers should not error for USDT margined futures")
}
...
func TestUpdateOrderbook(t *testing.T) {
	...
	assert.NoError(t, err, "UpdateOrderbook should not error for spot")

	_, err = e.UpdateOrderbook(t.Context(), cp, asset.Margin)
	assert.NoError(t, err, "UpdateOrderbook should not error for margin")

	_, err = e.UpdateOrderbook(t.Context(), cp, asset.USDTMarginedFutures)
	assert.NoError(t, err, "UpdateOrderbook should not error for USDT margined futures")

	cp2, err := currency.NewPairFromString("BTCUSD_PERP")
	require.NoError(t, err, "NewPairFromString must not error")

	_, err = e.UpdateOrderbook(t.Context(), cp2, asset.CoinMarginedFutures)
	assert.NoError(t, err, "UpdateOrderbook should not error for coin margined futures")
}
...
func TestUGetMarkPrice(t *testing.T) {
	...
	assert.NoError(t, err, "UGetMarkPrice should not error for a valid pair")

	_, err = e.UGetMarkPrice(t.Context(), currency.EMPTYPAIR)
	assert.NoError(t, err, "UGetMarkPrice should not error for an empty pair")
}
...
func TestU24HTickerPriceChangeStats(t *testing.T) {
	...
	assert.NoError(t, err, "U24HTickerPriceChangeStats should not error for a valid pair")

	_, err = e.U24HTickerPriceChangeStats(t.Context(), currency.EMPTYPAIR)
	assert.NoError(t, err, "U24HTickerPriceChangeStats should not error for an empty pair")
}

func TestUSymbolPriceTicker(t *testing.T) {
	t.Parallel()
	_, err := e.USymbolPriceTicker(t.Context(), currency.NewBTCUSDT())
	assert.NoError(t, err, "USymbolPriceTicker should not error for a valid pair")

	_, err = e.USymbolPriceTicker(t.Context(), currency.EMPTYPAIR)
	assert.NoError(t, err, "USymbolPriceTicker should not error for an empty pair")
}

func TestUSymbolOrderbookTicker(t *testing.T) {
	t.Parallel()
	_, err := e.USymbolOrderbookTicker(t.Context(), currency.NewBTCUSDT())
	assert.NoError(t, err, "USymbolOrderbookTicker should not error for a valid pair")

	_, err = e.USymbolOrderbookTicker(t.Context(), currency.EMPTYPAIR)
	assert.NoError(t, err, "USymbolOrderbookTicker should not error for an empty pair")
}
...
func TestUCompositeIndexInfo(t *testing.T) {
	...
	assert.NoError(t, err, "UCompositeIndexInfo should not error for a valid pair")

	_, err = e.UCompositeIndexInfo(t.Context(), currency.EMPTYPAIR)
	assert.NoError(t, err, "UCompositeIndexInfo should not error for an empty pair")
}
...
func TestUAllAccountOrders(t *testing.T) {
	...
	assert.NoError(t, err, "UAllAccountOrders should not error for an empty pair")

	_, err = e.UAllAccountOrders(t.Context(), currency.NewBTCUSDT(), 0, 5, time.Now().Add(-time.Hour*4), time.Now())
	assert.NoError(t, err, "UAllAccountOrders should not error with params")
}
...
func TestGetFuturesSwapTickerChangeStats(t *testing.T) {
	...
	assert.NoError(t, err, "GetFuturesSwapTickerChangeStats should not error for a valid pair")

	_, err = e.GetFuturesSwapTickerChangeStats(t.Context(), currency.NewPairWithDelimiter("BTCUSD", "PERP", "_"), "")
	assert.NoError(t, err, "GetFuturesSwapTickerChangeStats should not error for a valid pair again")

	_, err = e.GetFuturesSwapTickerChangeStats(t.Context(), currency.EMPTYPAIR, "")
	assert.NoError(t, err, "GetFuturesSwapTickerChangeStats should not error for an empty pair")
}
...
func TestGetFuturesHistoricalTrades(t *testing.T) {
	...
	assert.NoError(t, err, "GetFuturesHistoricalTrades with zero limit should not error")
}
...
func TestGetFuturesOrderbookTicker(t *testing.T) {
	...
	assert.NoError(t, err, "GetFuturesOrderbookTicker should not error for an empty pair")

	_, err = e.GetFuturesOrderbookTicker(t.Context(), currency.NewPairWithDelimiter("BTCUSD", "PERP", "_"), "")
	assert.NoError(t, err, "GetFuturesOrderbookTicker should not error for a valid pair")
}
...
func TestUGetNotionalLeverage(t *testing.T) {
	...
	assert.NoError(t, err, "FuturesNotionalBracket should not error for a valid symbol")

	_, err = e.FuturesNotionalBracket(t.Context(), "")
	assert.NoError(t, err, "FuturesNotionalBracket should not error for an empty symbol")
}
...
func TestFetchTradablePairs(t *testing.T) {
	...
	assert.NoError(t, err, "FetchTradablePairs should not error for spot")
...
func TestGetHistoricalTrades(t *testing.T) {
	...
	if mockTests {
		...
		assert.Error(t, err, "GetHistoricalTrades should error in live mode with invalid params")
	}
}
...
func TestGetTickers(t *testing.T) {
	...
	require.NoError(t, err, "GetTickers with pairs must not error")
	require.Len(t, resp, 2, "GetTickers with pairs must return 2 tickers")
}
...
func TestQueryOrder(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "QueryOrder should error when no credentials are set")
	}
}
...
func TestOpenOrders(t *testing.T) {
	...
	assert.NoError(t, err, "OpenOrders should not error for an empty pair")

	p := currency.NewBTCUSDT()
	_, err = e.OpenOrders(t.Context(), p)
	assert.NoError(t, err, "OpenOrders should not error for a valid pair")
}
...
func TestAllOrders(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		assert.NoError(t, err, "AllOrders should not error")
	} else {
		assert.Error(t, err, "AllOrders should error when no credentials are set")
	}
}
...
func TestGetFee(t *testing.T) {
	...
	if sharedtestvalues.AreAPICredentialsSet(e) && mockTests {
		...
		assert.NoError(t, err, "GetFee should not error for basic trade fee")
		...
		_, err = e.GetFee(t.Context(), feeBuilder)
		assert.NoError(t, err, "GetFee should not error for high quantity trade fee")

		// CryptocurrencyTradeFee IsMaker
		feeBuilder = setFeeBuilder()
		feeBuilder.IsMaker = true
		_, err = e.GetFee(t.Context(), feeBuilder)
		assert.NoError(t, err, "GetFee should not error for maker trade fee")
	...
	_, err = e.GetFee(t.Context(), feeBuilder)
	assert.NoError(t, err, "GetFee should not error for international bank deposit fee")
	...
	feeBuilder.FiatCurrency = currency.HKD
	_, err = e.GetFee(t.Context(), feeBuilder)
	assert.NoError(t, err, "GetFee should not error for international bank withdrawal fee")
}
...
func TestFormatWithdrawPermissions(t *testing.T) {
	...
	assert.Equal(t, expectedResult, withdrawPermissions, "Withdraw permissions should be formatted correctly")
}
...
func TestGetActiveOrders(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "GetActiveOrders should error when no credentials are set")
	}
}
...
func TestGetOrderHistory(t *testing.T) {
	...
	assert.Error(t, err, "GetOrderHistory should error when no pairs are provided")
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "GetOrderHistory should error when no credentials are set")
	}
}
...
func TestNewOrderTest(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "NewOrderTest should error when no credentials are set for limit order")
	}
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "NewOrderTest should error when no credentials are set for market order")
	}
}
...
func TestGetAggregatedTradesBatched(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err, "GetAggregatedTrades should not error")
			assert.Len(t, result, tt.numExpected, "GetAggregatedTradesBatched should return the expected number of entries")
			if len(result) > 0 {
				lastTradeTime := result[len(result)-1].TimeStamp
				assert.Equal(t, tt.lastExpected.UTC(), lastTradeTime.Time().UTC(), "Last trade time should match the expected time")
			}
		})
	}
}
...
func TestGetAggregatedTradesErrors(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Error(t, err, "GetAggregatedTrades should error for invalid params")
		})
	}
}
...
func TestSubmitOrder(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "SubmitOrder should error when no credentials are set")
	}
}
...
func TestCancelExchangeOrder(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "CancelOrder should error when no credentials are set")
	}
}
...
func TestCancelAllExchangeOrders(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "CancelAllOrders should error when no credentials are set")
	}
}
...
func TestWrapperGetActiveOrders(t *testing.T) {
	...
	assert.NoError(t, err, "GetActiveOrders should not error for coin margined futures")
	...
	assert.NoError(t, err, "GetActiveOrders should not error for USDT margined futures")
}
...
func TestWrapperGetOrderHistory(t *testing.T) {
	...
	assert.NoError(t, err, "GetOrderHistory should not error for coin margined futures")
	...
	assert.NoError(t, err, "GetOrderHistory should not error for USDT margined futures")

	_, err = e.GetOrderHistory(t.Context(), &order.MultiOrderRequest{
		AssetType: asset.USDTMarginedFutures,
	})
	assert.Error(t, err, "GetOrderHistory should error for invalid params")
}
...
func TestCancelOrder(t *testing.T) {
	...
	assert.NoError(t, err, "CancelOrder should not error for coin margined futures")
	...
	assert.NoError(t, err, "CancelOrder should not error for USDT margined futures")
}
...
func TestModifyOrder(t *testing.T) {
	...
	assert.Error(t, err, "ModifyOrder should error as it is not supported")
}
...
func TestWithdraw(t *testing.T) {
	...
	if sharedtestvalues.AreAPICredentialsSet(e) {
		assert.NoError(t, err, "WithdrawCryptocurrencyFunds should not error when credentials are set")
	} else {
		assert.Error(t, err, "WithdrawCryptocurrencyFunds should error when no credentials are set")
	}
}
...
func TestDepositHistory(t *testing.T) {
	...
	if sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "DepositHistory should error when no credentials are set")
	}
}
...
func TestWithdrawHistory(t *testing.T) {
	...
	if sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "GetWithdrawalsHistory should error when no credentials are set")
	}
}
...
func TestWithdrawFiat(t *testing.T) {
	...
	_, err := e.WithdrawFiatFunds(t.Context(),
		&withdraw.Request{})
	assert.ErrorIs(t, err, common.ErrFunctionNotSupported, "WithdrawFiatFunds should return function not supported error")
}
...
func TestWithdrawInternationalBank(t *testing.T) {
	...
	_, err := e.WithdrawFiatFundsToInternationalBank(t.Context(),
		&withdraw.Request{})
	assert.ErrorIs(t, err, common.ErrFunctionNotSupported, "WithdrawFiatFundsToInternationalBank should return function not supported error")
}
...
func TestGetDepositAddress(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		...
		assert.Error(t, err, "GetDepositAddress should error when no credentials are set")
	}
}
...
func TestWsTickerUpdate(t *testing.T) {
	...
	assert.NoError(t, err, "wsHandleData should not error for ticker update")
}
...
func TestWsKlineUpdate(t *testing.T) {
	...
	err := e.wsHandleData(pressXToJSON)
	assert.NoError(t, err, "wsHandleData should not error for kline update")
}
...
func TestWsTradeUpdate(t *testing.T) {
	...
	err := e.wsHandleData(pressXToJSON)
	assert.NoError(t, err, "wsHandleData should not error for trade update")
}
...
func TestWsDepthUpdate(t *testing.T) {
	...
	require.NoError(t, e.wsHandleData(update1), "wsHandleData must not error for update1")
	...
	assert.Equal(t, seedLastUpdateID, ob.LastUpdateID, "Last update id of orderbook should not be updated by old update")
	assert.Equal(t, 2.3, ob.Asks[2].Amount, "Ask should not be altered by outdated update")
	assert.Equal(t, 0.163526, ob.Bids[1].Amount, "Bid should not be altered by outdated update")
	...
	assert.NoError(t, e.wsHandleData(update2), "wsHandleData should not error for update2")

	ob, err = e.Websocket.Orderbook.GetOrderbook(p, asset.Spot)
	require.NoError(t, err, "GetOrderbook must not error after update2")
	assert.Equal(t, int64(165), ob.LastUpdateID, "Last update id of orderbook should be updated by new update")
	assert.Equal(t, 2.3, ob.Asks[2].Amount, "Ask amount should be updated correctly")
	assert.Equal(t, 1.9, ob.Asks[3].Amount, "Ask amount should be updated correctly")
	assert.Equal(t, 0.163526, ob.Bids[1].Amount, "Bid amount should be updated correctly")
...
func TestWsBalanceUpdate(t *testing.T) {
  ...
  "T": 1573200697068}}`)
	err := e.wsHandleData(pressXToJSON)
	assert.NoError(t, err, "wsHandleData should not error for balance update")
}
...
func TestWsOCO(t *testing.T) {
  ...
  "O": [                            
	...
	err := e.wsHandleData(pressXToJSON)
	assert.NoError(t, err, "wsHandleData should not error for OCO update")
}
...
func TestMaintainWsAuthStreamKey(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		assert.NoError(t, err, "MaintainWsAuthStreamKey should not error")
	} else {
...
func TestExecutionTypeToOrderStatus(t *testing.T) {
	...
	for i := range testCases {
		...
		assert.Equal(t, testCases[i].Result, result, "Order status should be correct for case "+testCases[i].Case)
	}
}
...
func TestGetRecentTrades(t *testing.T) {
	...
	_, err := e.GetRecentTrades(t.Context(),
		pair, asset.Spot)
	assert.NoError(t, err, "GetRecentTrades should not error for spot")

	_, err = e.GetRecentTrades(t.Context(),
		pair, asset.USDTMarginedFutures)
	assert.NoError(t, err, "GetRecentTrades should not error for USDT margined futures")

	pair.Base = currency.NewCode("BTCUSD")
	pair.Quote = currency.PERP
	_, err = e.GetRecentTrades(t.Context(),
		pair, asset.CoinMarginedFutures)
	assert.NoError(t, err, "GetRecentTrades should not error for coin margined futures")
}
...
func TestGetAvailableTransferChains(t *testing.T) {
	...
	if mockTests || sharedtestvalues.AreAPICredentialsSet(e) {
		assert.NoError(t, err, "GetAvailableTransferChains should not error")
	} else {
		assert.Error(t, err, "GetAvailableTransferChains should error when no credentials are set")
	}
}
