...
func configureWsConnectAuth(t *testing.T, ex *Exchange, listenKeyValue string, subsequentListenKeyValues ...string) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, http.MethodPost, r.Method, "auth stream request method should match")
...
func TestKeepAuthKeyAlive(t *testing.T) {
	ex := newWsConnectTestExchange(t, new(wsConnectTestConnection))
	close(ex.Websocket.ShutdownC)
	ex.KeepAuthKeyAlive(t.Context())
}
...
func TestWsConnect(t *testing.T) {
	...
	t.Run("authenticated dial failure keeps listen key ephemeral", func(t *testing.T) {
		const (
			...
			persistentURL   = binanceDefaultWebsocketURL + "?streams=btcusdt@trade/ethusdt@trade&timeUnit=MICROSECOND"
		)
		listenKey = ""
	...
	t.Run("stale URL reset failure", func(t *testing.T) {
		...
		conn := new(wsConnectTestConnection)
		ex := newWsConnectTestExchange(t, conn)
		configureWsConnectMultiConnectionManager(t, ex, conn, binanceDefaultWebsocketURL+"?streams="+staleListenKeyCanary)
	...
	t.Run("authenticated public URL survives managed connection", func(t *testing.T) {
		const persistentURL = binanceDefaultWebsocketURL + "?streams=btcusdt@trade/ethusdt@trade&mode=combined#public-fragment"
		listenKey = ""
		conn := new(wsConnectTestConnection)
		ex := newWsConnectTestExchange(t, conn)
	...
	t.Run("auth key failure preserves public combined streams", func(t *testing.T) {
		const (
			...
			combinedStreamURL = binanceDefaultWebsocketURL + "?streams=btcusdt@trade/ethusdt@trade&mode=combined&opaque=one%20two#public-fragment"
		)
	...
	t.Run("authenticated reconnect keeps stored public URL while wire streams use fresh keys", func(t *testing.T) {
		const (
			...
			publicStreamsURL = binanceDefaultWebsocketURL + "?streams=btcusdt@trade/ethusdt@trade&mode=combined&opaque=one%20two#public-fragment"
		)
		listenKey = ""
		conn := new(wsConnectTestConnection)
		ex := newWsConnectTestExchange(t, conn)
	...
	t.Run("public combined stream URL is preserved", func(t *testing.T) {
		const combinedStreamURL = binanceDefaultWebsocketURL + "?streams=btcusdt@trade/ethusdt@trade"
