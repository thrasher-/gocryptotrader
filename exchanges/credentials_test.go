...
func TestAPISetters(t *testing.T) {
	...
	assert.Equal(t, account.Key, api.credentials.Key, "SetKey should set API key")

	api = API{}
	api.SetSecret(account.Secret)
	assert.Equal(t, account.Secret, api.credentials.Secret, "SetSecret should set secret")

	api = API{}
	api.SetClientID(account.ClientID)
	assert.Equal(t, account.ClientID, api.credentials.ClientID, "SetClientID should set client id")

	api = API{}
	api.SetPEMKey(account.PEMKey)
	assert.Equal(t, account.PEMKey, api.credentials.PEMKey, "SetPEMKey should set pem key")

	api = API{}
	api.SetSubAccount(account.SubAccountSTR)
	assert.Equal(t, account.SubAccountSTR, api.credentials.SubAccount, "SetSubAccount should set sub account")
}
