...
func TestProtectedString(t *testing.T) {
	t.Parallel()
	p := Protected{}
	assert.Equal(t, "Key:[...] SubAccount:[] ClientID:[]", p.String(), "Protected.String should mask values")

	p.creds.Key = "12345678910111234"
	p.creds.SubAccount = "sub"
	p.creds.ClientID = "client"

	assert.Equal(t, "Key:[1234567891011123...] SubAccount:[sub] ClientID:[client]", p.creds.String(), "Protected.String should format credential values")
}
...
func TestProtectedCredentialsEqual(t *testing.T) {
	...
	assert.False(t, this.Equal(that), "Protected.Equal should return false when protected creds empty and other nil")
	this.creds = Credentials{}
	assert.False(t, this.Equal(that), "Protected.Equal should return false when protected creds empty")
	that = &Credentials{Key: "1337"}
	assert.False(t, this.Equal(that), "Protected.Equal should return false when keys differ")
	this.creds.Key = "1337"
	assert.True(t, this.Equal(that), "Protected.Equal should return true when keys match")
	this.creds.ClientID = "1337"
	assert.False(t, this.Equal(that), "Protected.Equal should return false when client ids differ")
	that.ClientID = "1337"
	assert.True(t, this.Equal(that), "Protected.Equal should return true when keys and client ids match")
	this.creds.SubAccount = "someSub"
	assert.False(t, this.Equal(that), "Protected.Equal should return false when sub accounts differ")
	that.SubAccount = "someSub"
	assert.True(t, this.Equal(that), "Protected.Equal should return true when all credentials match")
}
