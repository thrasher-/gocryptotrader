...
func TestLiveUnsubscribe(t *testing.T) {
	if livetest.ShouldSkip() {
		t.Skipf(livetest.LiveTestingSkipped, e.Name)
	}
	ex := new(Exchange)
	...
	require.NoError(t, ex.Subscribe(subs), "Subscribe must receive acknowledgements before Unsubscribe")
	require.Len(t, ex.Websocket.GetSubscriptions(), len(subs), "Websocket.GetSubscriptions must contain both subscriptions before Unsubscribe")
