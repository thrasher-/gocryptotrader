...
`TriggerPriceType: order.MarkPrice`. If the parent succeeds and a grouped child
is rejected, `SubmitOrder` returns the accepted parent with `SubmissionError`
set. Callers must inspect that field before assuming the position is protected.

Online fee estimates use the configured account's tier rates from `userFees`.
Offline or credentialless estimates use Hyperliquid's published base-tier
maker/taker rates. These estimates do not apply market-specific HIP-3 deployer
or growth-mode adjustments, quote-to-quote spot discounts, aligned-quote
adjustments, or order builder fees. When the same display pair exists on Spot
and Perpetuals, the enabled asset configuration selects the applicable fee
schedule; enabling both remains ambiguous and fails closed.
...
Account funding and withdrawal history read the complete ledger. Large accounts
may need several minutes of paginated requests within the exchange rate limit.
Use `GetUserNonFundingLedgerUpdates` with `*UserLedgerRequest` for a bounded range.
...
The test setup follows the shared exchange test helpers and loads
`testdata/configtest.json` without making API calls. Tests named `TestLive...`
query Hyperliquid. Mock tests in `*_mock_test.go` use local HTTP/Websocket
fixtures with explicit request and response assertions.
Signing and transfer tests use deterministic offline fixtures.
Each method has a separate test function; subtests cover inputs or assets for
that method.

Run all Hyperliquid tests, including public live checks:

```sh
go test ./exchanges/hyperliquid -race -count=1
```

Run only offline tests:

```sh
go test ./exchanges/hyperliquid -race -count=1 -skip '^TestLive'
```

Run only live tests:

```sh
go test ./exchanges/hyperliquid -race -count=1 -run '^TestLive'
```

Run a single method's live test:

```sh
go test ./exchanges/hyperliquid -race -count=1 -run '^TestLiveGetAllMids$'
```

The shared `GCT_SKIP_LIVE_TESTS=true` setting also skips live checks.

Address-scoped live tests require `apiCredentials.Key` in `hyperliquid_test.go`.
They do not need a private key. Without an address these tests are skipped.
Public live tests cover Spot, default perpetual markets, a registered HIP-3
DEX, and Websocket subscription, data delivery, unsubscription and shutdown.
