...
func (b *Bitmex) wsHandleData(respRaw []byte) error {
	...
	case version != "":
		...
		return nil
	case errMsg != "", success:
		if err := b.Websocket.Match.RequireMatchWithData(op, respRaw); err != nil {
...
func (b *Bitmex) manageSubs(op string, subs subscription.List) error {
	...
	resps, errs := b.Websocket.Conn.SendMessageReturnResponses(context.TODO(), request.Unset, op, req, len(subs))
